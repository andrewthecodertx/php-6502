...
$memory->initialize([
    ...
    0x8000 => 0xA9, 0x8001 => 0x42,  // LDA #$42      ; Load 0x42 into A
    0x8002 => 0xAA,                  // TAX           ; Transfer A to X
    0x8003 => 0xA9, 0x8004 => 0x84,  // LDA #$84      ; Load 0x84 into A
    0x8005 => 0xA8,                  // TAY           ; Transfer A to Y
    0x8006 => 0x8A,                  // TXA           ; Transfer X to A
    0x8007 => 0x85, 0x8008 => 0x80,  // STA $80       ; Store A to zero page
    0x8009 => 0x98,                  // TYA           ; Transfer Y to A
    0x800A => 0x86, 0x800B => 0x81,  // STX $81       ; Store X to zero page
    0x800C => 0x84, 0x800D => 0x82,  // STY $82       ; Store Y to zero page
    0x800E => 0xA6, 0x800F => 0x80,  // LDX $80       ; Load X from zero page
    0x8010 => 0xA4, 0x8011 => 0x82,  // LDY $82       ; Load Y from zero page
...
foreach ($instructions as $address => $instruction) {
    ...
    echo sprintf("Before: PC=0x%04X A=0x%02X X=0x%02X Y=0x%02X\n",
        $cpu->pc, $cpu->getAccumulator(), $cpu->getRegisterX(), $cpu->getRegisterY());
    ...
    echo sprintf("After:  PC=0x%04X A=0x%02X X=0x%02X Y=0x%02X\n",
        $cpu->pc, $cpu->getAccumulator(), $cpu->getRegisterX(), $cpu->getRegisterY());
