...
class DisplayTimingTest extends TestCase
{
    ...
    public function testConsoleTicksDisplay(): void
    {
        ...
        $this->assertEquals(ord('B'), $this->display->read(TextDisplay::DISPLAY_BASE));
    }

    public function testBusIntegrityAfterTicking(): void
    {
        $this->bus->write(TextDisplay::DISPLAY_BASE, ord('C'));
        ...
        $this->assertEquals(ord('C'), $this->bus->read(TextDisplay::DISPLAY_BASE));
    ...
    public function testCPUStepWithDisplayWrites(): void
    {
        ...
        $this->assertEquals(ord('T'), $this->display->read(TextDisplay::DISPLAY_BASE + 0));
        $this->assertEquals(ord('E'), $this->display->read(TextDisplay::DISPLAY_BASE + 1));
        $this->assertEquals(ord('S'), $this->display->read(TextDisplay::DISPLAY_BASE + 2));
        $this->assertEquals(ord('T'), $this->display->read(TextDisplay::DISPLAY_BASE + 3));
    }

    public function testContinuousDisplayOperations(): void
    {
        for ($i = 0; $i < 40; $i++) {
            $this->display->write(TextDisplay::DISPLAY_BASE + $i, ord('A') + ($i % 26));
        }
    ...
    {
        $testData = [
            ['address' => TextDisplay::CURSOR_X, 'value' => 15],
            ['address' => TextDisplay::CURSOR_Y, 'value' => 10],
            ['address' => TextDisplay::FG_COLOR, 'value' => 12],
            ['address' => TextDisplay::BG_COLOR, 'value' => 3],
            ['address' => TextDisplay::DISPLAY_BASE + 100, 'value' => ord('X')],
        ];
